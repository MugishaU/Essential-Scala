...
//mcTiernan.films.foldLeft(2020)((current, film) => math.min(current, film.yearOfRelease))
//directors.flatMap(_.films).sortWith(_.imdbRating > _.imdbRating).map(_.name)
//val films = directors.flatMap(_.films)
//films.foldLeft(0.0)((sum, film) => sum + film.imdbRating) / films.length

directors.map(dir => dir.films.map(film => println(s"Tonight only! ${film.name} by ${dir.lastName}!")))
